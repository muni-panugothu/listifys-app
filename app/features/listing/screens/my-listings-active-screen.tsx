...
export function MyListingsActiveScreen() {
  ...
  const loadListings = useCallback(async () => {
    ...
    try {
      ...
      setListings((res.listings || []).filter((l) => l.status === "active" || !l.status));
    } catch {
      // keep existing
    } finally {
      setLoading(false);
        ...
        <View className="px-4">
          ...
          {!loading && listings.length === 0 && (
            <View className="items-center py-16">
              ...
              <Text className="mt-3 text-[16px] font-semibold text-[#6C7A74]">No active listings</Text>
              <Text className="mt-1 text-[13px] text-[#94A3B8]">Post your first item to see it here</Text>
              <Pressable onPress={() => router.push("/sell-entry")} className="mt-4 rounded-lg bg-[#27BB97] px-6 py-2.5">
                <Text className="text-[14px] font-semibold text-white">Start Selling</Text>
              </Pressable>
            </View>
          )}
            ...
            {listings.map((listing) => (
              ...
              >
                ...
                <View className="relative h-40 w-full">
                  {listing.images?.[0] ? (
                    <Image source={listing.images[0]} contentFit="cover" className="h-full w-full" />
                  ) : (
                ...
                <View className="p-4">
                  ...
                  <Text className="mt-1 text-[12px] font-medium text-[#6C7A74]">
                    {(listing as any)._source ?? listing.category} • Posted {timeAgo(listing.createdAt)}
                  </Text>
                  {/* Stats */}
