...
exports.createBeauty = async (req, res) => {
  try {
    ...
    const populated = await Beauty.findById(listing._id).populate(
      "seller",
      "name profileImage",
    );
    ...
    res.status(201).json({
      success: true,
      message: "Beauty listing created successfully",
      listing: listingObj,
    });
    ...
    logger.error("Create beauty error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to create beauty listing" });
  }
};
...
exports.getAllBeauty = async (req, res) => {
  try {
    ...
    const cached = await ListingCache.getCachedListingList("beauty", queryKey);
    if (cached) {
    ...
    if (search && !(lat && lng)) {
      ...
      if (esResult) {
        ...
        Promise.all([
          ListingCache.cacheListingList('beauty', queryKey, esResult.docs, esResult.pagination),
          ListingCache.prefetchCategoryListings('beauty', esResult.docs),
          ListingCache.cacheSearchResults('beauty', search, esResult.docs, esResult.pagination),
    ...
    if (safePage > 1) {
      listings = await Beauty.find(filter, LIST_PROJECTION)
        .sort(sortOption)
        .skip(skip)
        .limit(safeLimit + 1)
      ...
      const [items, total] = await Promise.all([
        Beauty.find(filter, LIST_PROJECTION)
          .sort(sortOption)
          .limit(safeLimit)
          .populate("seller", "name profileImage")
          .lean(),
        Beauty.countDocuments(filter),
      ]);
      listings = items;
      pagination = {
        total,
    ...
    Promise.all([
      ListingCache.cacheListingList("beauty", queryKey, listings, pagination),
      ListingCache.prefetchCategoryListings("beauty", listings),
      search
        ? ListingCache.cacheSearchResults("beauty", search, listings, pagination)
        : null,
    ]).catch((err) =>
      logger.error(
        "[Cache] Background beauty cache write error:",
        err.message,
      ),
    );
    ...
    logger.error("Get beauty error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to fetch beauty listings" });
  }
};
...
exports.getBeautyById = async (req, res) => {
  try {
    ...
    if (isObjectId) {
      const cached = await ListingCache.getCachedListing("beauty", param);
      if (cached) {
        viewCounter.recordView("beauty", param);
    ...
    const listing = isObjectId
      ? await Beauty.findById(param).populate(
          "seller",
          "name profileImage",
        )
      : await Beauty.findOne({ slug: param, status: "active" }).populate(
          "seller",
          "name profileImage",
        );
    ...
    if (!listing || listing.status !== "active") {
      return res
        .status(404)
        .json({ success: false, message: "Beauty listing not found" });
    }

    const listingId = listing._id.toString();
    viewCounter.recordView("beauty", listingId);
    ...
    ListingCache.cacheListing("beauty", listingObj).catch((err) =>
      logger.error(
        "[Cache] Background beauty detail cache error:",
        err.message,
      ),
    );
    ...
    logger.error("Get beauty by id error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to fetch beauty listing" });
  }
};
...
exports.updateBeauty = async (req, res) => {
  try {
    ...
    const listing = await Beauty.findById(req.params.id);
    if (!listing) {
      return res
        .status(404)
        .json({ success: false, message: "Beauty listing not found" });
    ...
    Object.assign(listing, { ...req.body, category: "Beauty" });
    await listing.save();

    const updated = await Beauty.findById(listing._id).populate(
      "seller",
      "name profileImage",
    );
    ...
    try {
      await Promise.all([
        ListingCache.cacheListing("beauty", listingObj),
        ListingCache.invalidateListCaches("beauty"),
      ]);
    } catch (cacheErr) {
      logger.error(
        "[Cache] Beauty update cache sync error:",
        cacheErr.message,
      );
    }
    ...
    logger.error("Update beauty error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to update beauty listing" });
  }
};
...
exports.deleteBeauty = async (req, res) => {
  try {
    ...
    try {
      await Promise.all([
        ListingCache.invalidateListCaches("beauty"),
        ListingCache.invalidateListingCache("beauty", req.params.id),
      ]);
    } catch (cacheErr) {
      logger.error(
        "[Cache] Beauty delete cache sync error:",
        cacheErr.message,
      );
    }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Beauty listing deleted" });
  } catch (error) {
    logger.error("Delete beauty error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to delete beauty listing" });
  }
};

exports.getMyBeauty = async (req, res) => {
  try {
    ...
    logger.error("Get my beauty error:", error);
    return res.status(500).json({
      success: false,
      message: "Failed to fetch your beauty listings",
    });
  }
};

exports.getSavedBeauty = async (req, res) => {
  try {
    const { page, limit } = parsePagination(req.query, { limit: 20 });

    const savedKey = `saved:beauty:${req.user._id}:p${page}:l${limit}`;
    try {
      ...
      logger.warn("Saved beauty cache read failed", {
    ...
    try {
      ...
      logger.warn("Saved beauty cache write failed", {
        error: cacheErr.message,
      });
    }
    ...
    logger.error("Get saved beauty error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to fetch saved beauty" });
  }
};
...
exports.uploadImages = async (req, res) => {
  try {
    ...
    logger.error("Upload beauty images error:", error);
    return res
      .status(500)
...
exports.toggleSave = async (req, res) => {
  try {
    ...
    const listing = await Beauty.findById(req.params.id).select('savedBy').lean();
    if (!listing) {
      return res
        .status(404)
        .json({ success: false, message: "Beauty listing not found" });
    ...
    if (alreadySaved) {
      await Beauty.updateOne({ _id: req.params.id }, { $pull: { savedBy: userId } });
    } else {
      await Beauty.updateOne({ _id: req.params.id }, { $addToSet: { savedBy: userId } });
    }

    return res.status(200).json({
      success: true,
      saved: !alreadySaved,
      message: !alreadySaved ? "Beauty item saved" : "Beauty item unsaved",
    });
  } catch (error) {
    logger.error("Toggle save beauty error:", error);
    return res
      .status(500)
