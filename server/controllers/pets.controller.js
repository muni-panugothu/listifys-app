...
exports.createPet = async (req, res) => {
  try {
    ...
    const listing = await Pet.create(payload);
    const populated = await Pet.findById(listing._id).populate(
      "seller",
      "name profileImage",
    );
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to create pet listing" });
  }
};
...
exports.getAllPets = async (req, res) => {
  try {
    ...
    const cached = await ListingCache.getCachedListingList("pets", queryKey);
    if (cached) {
    ...
    if (search && !(lat && lng)) {
      ...
      if (esResult) {
        ...
        Promise.all([
          ListingCache.cacheListingList('pets', queryKey, esResult.docs, esResult.pagination),
          ListingCache.prefetchCategoryListings('pets', esResult.docs),
          ListingCache.cacheSearchResults('pets', search, esResult.docs, esResult.pagination),
    ...
    if (safePage > 1) {
      listings = await Pet.find(filter, LIST_PROJECTION)
        .sort(sortOption)
        .skip(skip)
        .limit(safeLimit + 1)
      ...
      const [items, total] = await Promise.all([
        Pet.find(filter, LIST_PROJECTION)
          .sort(sortOption)
          .limit(safeLimit)
          .populate("seller", "name profileImage")
          .lean(),
    ...
    Promise.all([
      ListingCache.cacheListingList("pets", queryKey, listings, pagination),
      ListingCache.prefetchCategoryListings("pets", listings),
      search
        ? ListingCache.cacheSearchResults("pets", search, listings, pagination)
        : null,
    ]).catch((err) =>
      logger.error(
        "[Cache] Background pets cache write error:",
        err.message,
      ),
    );
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to fetch pets" });
  }
};
...
exports.getPetById = async (req, res) => {
  try {
    ...
    if (isObjectId) {
      const cached = await ListingCache.getCachedListing("pets", param);
      if (cached) {
        viewCounter.recordView("pets", param);
    ...
    const listing = isObjectId
      ? await Pet.findById(param).populate(
          "seller",
          "name profileImage",
        )
      : await Pet.findOne({ slug: param, status: "active" }).populate(
          "seller",
          "name profileImage",
        );
    ...
    viewCounter.recordView("pets", listingId);
    ...
    ListingCache.cacheListing("pets", listingObj).catch((err) =>
      logger.error(
        "[Cache] Background pets detail cache error:",
        err.message,
      ),
    );
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to fetch pet listing" });
  }
};
...
exports.updatePet = async (req, res) => {
  try {
    ...
    const listing = await Pet.findById(req.params.id);
    if (!listing) {
      return res
        .status(404)
        .json({ success: false, message: "Pet listing not found" });
    ...
    Object.assign(listing, { ...req.body, category: "Pets" });
    await listing.save();

    const updated = await Pet.findById(listing._id).populate(
      "seller",
      "name profileImage",
    );
    ...
    try {
      await Promise.all([
        ListingCache.cacheListing("pets", listingObj),
        ListingCache.invalidateListCaches("pets"),
      ]);
    } catch (cacheErr) {
      logger.error("[Cache] Pets update cache sync error:", cacheErr.message);
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Failed to update pet listing" });
  }
};
...
exports.deletePet = async (req, res) => {
  try {
    ...
    try {
      await Promise.all([
        ListingCache.invalidateListCaches("pets"),
        ListingCache.invalidateListingCache("pets", req.params.id),
      ]);
    } catch (cacheErr) {
      logger.error("[Cache] Pets delete cache sync error:", cacheErr.message);
    }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Pet listing deleted" });
  } catch (error) {
    logger.error("Delete pet error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to delete pet listing" });
  }
};
...
exports.getMyPets = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      message: "Failed to fetch your pet listings",
    });
  }
};
...
exports.getSavedPets = async (req, res) => {
  try {
    ...
    const savedKey = `saved:pets:${req.user._id}:p${page}:l${limit}`;
    try {
      ...
      logger.warn("Saved pets cache read failed", {
    ...
    try {
      ...
      logger.warn("Saved pets cache write failed", {
        error: cacheErr.message,
      });
    }
    ...
    logger.error("Get saved pets error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to fetch saved pets" });
  }
};
...
exports.uploadImages = async (req, res) => {
  try {
    ...
    try {
      ...
      logger.error("[Cache] Error caching uploaded pets images:", cacheErr.message);
    }

    return res.status(200).json({ success: true, imageUrls });
  } catch (error) {
    logger.error("Upload pets images error:", error);
    return res
      .status(500)
...
exports.toggleSave = async (req, res) => {
  try {
    ...
    const listing = await Pet.findById(req.params.id).select('savedBy').lean();
    if (!listing) {
      return res
        .status(404)
        .json({ success: false, message: "Pet listing not found" });
    ...
    if (alreadySaved) {
      await Pet.updateOne({ _id: req.params.id }, { $pull: { savedBy: userId } });
    } else {
      await Pet.updateOne({ _id: req.params.id }, { $addToSet: { savedBy: userId } });
    }

    try {
      const pattern = `saved:pets:${userId}:*`;
      const keys = await redis.keys(pattern);
      if (keys.length) await redis.del(keys);
    } catch (cacheErr) {
      logger.warn("Toggle save pets cache clear failed", {
        error: cacheErr.message,
      });
    }
    ...
    logger.error("Toggle save pet error:", error);
    return res
      .status(500)
