...
exports.createFurniture = async (req, res) => {
  try {
    ...
    const populated = await Furniture.findById(listing._id).populate(
      "seller",
      "name profileImage",
    );
    ...
    res.status(201).json({
      success: true,
      message: "Furniture listing created successfully",
      listing: listingObj,
    });
    ...
    logger.error("Create furniture error:", error);
    return res.status(500).json({ success: false, message: "Failed to create furniture listing" });
  }
};
...
exports.getAllFurniture = async (req, res) => {
  try {
    ...
    const cached = await ListingCache.getCachedListingList("furniture", queryKey);
    if (cached) {
    ...
    if (search && !(lat && lng)) {
      ...
      if (esResult) {
        ...
        Promise.all([
          ListingCache.cacheListingList('furniture', queryKey, esResult.docs, esResult.pagination),
          ListingCache.prefetchCategoryListings('furniture', esResult.docs),
          ListingCache.cacheSearchResults('furniture', search, esResult.docs, esResult.pagination),
    ...
    if (safePage > 1) {
      listings = await Furniture.find(filter, LIST_PROJECTION)
        .sort(sortOption)
        .skip(skip)
        .limit(safeLimit + 1)
      ...
      const [items, total] = await Promise.all([
        Furniture.find(filter, LIST_PROJECTION)
          .sort(sortOption)
          .limit(safeLimit)
          .populate("seller", "name profileImage")
          .lean(),
    ...
    Promise.all([
      ListingCache.cacheListingList("furniture", queryKey, listings, pagination),
      ListingCache.prefetchCategoryListings("furniture", listings),
      search
        ? ListingCache.cacheSearchResults("furniture", search, listings, pagination)
        : null,
    ]).catch((err) =>
      logger.error("[Cache] Background furniture cache write error:", err.message),
    );

    return;
  } catch (error) {
    logger.error("Get furniture error:", error);
    return res.status(500).json({ success: false, message: "Failed to fetch furniture" });
  }
};
...
exports.getFurnitureById = async (req, res) => {
  try {
    ...
    if (isObjectId) {
      const cached = await ListingCache.getCachedListing("furniture", param);
      if (cached) {
        viewCounter.recordView("furniture", param);
    ...
    const listing = isObjectId
      ? await Furniture.findById(param).populate("seller", "name profileImage")
      : await Furniture.findOne({ slug: param, status: "active" }).populate("seller", "name profileImage");

    if (!listing || listing.status !== "active") {
      return res.status(404).json({ success: false, message: "Furniture listing not found" });
    }

    const listingId = listing._id.toString();
    viewCounter.recordView("furniture", listingId);
    ...
    ListingCache.cacheListing("furniture", listingObj).catch((err) =>
      logger.error("[Cache] Background furniture detail cache error:", err.message),
    );

    return;
  } catch (error) {
    logger.error("Get furniture by id error:", error);
    return res.status(500).json({ success: false, message: "Failed to fetch furniture listing" });
  }
};
...
exports.updateFurniture = async (req, res) => {
  try {
    ...
    const listing = await Furniture.findById(req.params.id);
    if (!listing) {
      return res.status(404).json({ success: false, message: "Furniture listing not found" });
    }
    ...
    Object.assign(listing, { ...req.body, category: "Furniture" });
    await listing.save();

    const updated = await Furniture.findById(listing._id).populate(
      "seller",
      "name profileImage",
    );
    ...
    try {
      await Promise.all([
        ListingCache.cacheListing("furniture", listingObj),
        ListingCache.invalidateListCaches("furniture"),
      ]);
    } catch (cacheErr) {
      logger.error("[Cache] Furniture update cache sync error:", cacheErr.message);
    }
    ...
    return res.status(500).json({ success: false, message: "Failed to update furniture listing" });
  }
};
...
exports.deleteFurniture = async (req, res) => {
  try {
    ...
    try {
      await Promise.all([
        ListingCache.invalidateListCaches("furniture"),
        ListingCache.invalidateListingCache("furniture", req.params.id),
      ]);
    } catch (cacheErr) {
      logger.error("[Cache] Furniture delete cache sync error:", cacheErr.message);
    }
    ...
    return res.status(200).json({ success: true, message: "Furniture listing deleted" });
  } catch (error) {
    logger.error("Delete furniture error:", error);
    return res.status(500).json({ success: false, message: "Failed to delete furniture listing" });
  }
};
...
exports.getMyFurniture = async (req, res) => {
  try {
    ...
    logger.error("Get my furniture error:", error);
    return res.status(500).json({ success: false, message: "Failed to fetch your furniture listings" });
  }
};

exports.getSavedFurniture = async (req, res) => {
  try {
    const { page, limit } = parsePagination(req.query, { limit: 20 });

    const savedKey = `saved:furniture:${req.user._id}:p${page}:l${limit}`;
    try {
      ...
      logger.debug("Saved furniture cache miss:", cacheErr.message);
    }
    ...
    try {
      ...
      logger.error("[Cache] Error caching saved furniture:", cacheErr.message);
    }

    res.setHeader("X-Cache", "MISS");
    return res.status(200).json({ success: true, listings: items, pagination });
  } catch (error) {
    logger.error("Get saved furniture error:", error);
    return res.status(500).json({ success: false, message: "Failed to fetch saved furniture" });
  }
};
...
exports.uploadImages = async (req, res) => {
  try {
    ...
    try {
      ...
      logger.error("[Cache] Error caching uploaded furniture images:", cacheErr.message);
    }

    return res.status(200).json({ success: true, imageUrls });
  } catch (error) {
    logger.error("Upload furniture images error:", error);
...
exports.toggleSave = async (req, res) => {
  try {
    ...
    const listing = await Furniture.findById(req.params.id).select('savedBy').lean();
    if (!listing) {
      return res.status(404).json({ success: false, message: "Furniture listing not found" });
    }
    ...
    if (alreadySaved) {
      await Furniture.updateOne({ _id: req.params.id }, { $pull: { savedBy: userId } });
    } else {
      await Furniture.updateOne({ _id: req.params.id }, { $addToSet: { savedBy: userId } });
    }

    try {
      const savedKeyPattern = `saved:furniture:${userId}:*`;
      const keys = await redis.keys(savedKeyPattern);
      if (keys?.length) {
        await redis.del(...keys);
      }
      await ListingCache.invalidateListingCache("furniture", req.params.id);
    } catch (cacheErr) {
      logger.error("[Cache] Error updating furniture save cache:", cacheErr.message);
    }
    ...
    logger.error("Toggle save furniture error:", error);
