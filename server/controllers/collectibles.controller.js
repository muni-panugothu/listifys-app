...
exports.createCollectible = async (req, res) => {
  try {
    ...
    const listing = await Collectible.create(payload);
    const populated = await Collectible.findById(listing._id).populate(
      "seller",
      "name profileImage",
    );
    ...
    res.status(201).json({
      success: true,
      message: "Collectible listing created successfully",
      listing: listingObj,
    });
    ...
    logger.error("Create collectible error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to create collectible listing" });
  }
};
...
exports.getAllCollectibles = async (req, res) => {
  try {
    ...
    const cached = await ListingCache.getCachedListingList("collectibles", queryKey);
    if (cached) {
    ...
    if (search && !(lat && lng)) {
      ...
      if (esResult) {
        ...
        Promise.all([
          ListingCache.cacheListingList('collectibles', queryKey, esResult.docs, esResult.pagination),
          ListingCache.prefetchCategoryListings('collectibles', esResult.docs),
          ListingCache.cacheSearchResults('collectibles', search, esResult.docs, esResult.pagination),
    ...
    if (safePage > 1) {
      listings = await Collectible.find(filter, LIST_PROJECTION)
        .sort(sortOption)
        .skip(skip)
        .limit(safeLimit + 1)
      ...
      const [items, total] = await Promise.all([
        Collectible.find(filter, LIST_PROJECTION)
          .sort(sortOption)
          .limit(safeLimit)
          .populate("seller", "name profileImage")
          .lean(),
    ...
    Promise.all([
      ListingCache.cacheListingList("collectibles", queryKey, listings, pagination),
      ListingCache.prefetchCategoryListings("collectibles", listings),
      search
        ? ListingCache.cacheSearchResults("collectibles", search, listings, pagination)
        : null,
    ]).catch((err) =>
      logger.error(
        "[Cache] Background collectibles cache write error:",
        err.message,
      ),
    );
    ...
    logger.error("Get collectibles error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to fetch collectibles" });
  }
};

exports.getCollectibleById = async (req, res) => {
  try {
    const param = req.params.id;
    const isObjectId = mongoose.Types.ObjectId.isValid(param);

    if (isObjectId) {
      const cached = await ListingCache.getCachedListing("collectibles", param);
      if (cached) {
        viewCounter.recordView("collectibles", param);
    ...
    const listing = isObjectId
      ? await Collectible.findById(param).populate(
          "seller",
          "name profileImage",
        )
      : await Collectible.findOne({ slug: param, status: "active" }).populate(
          "seller",
          "name profileImage",
        );
    ...
    if (!listing || listing.status !== "active") {
      return res
        .status(404)
        .json({ success: false, message: "Collectible listing not found" });
    }

    const listingId = listing._id.toString();
    viewCounter.recordView("collectibles", listingId);
    ...
    ListingCache.cacheListing("collectibles", listingObj).catch((err) =>
      logger.error(
        "[Cache] Background collectibles detail cache error:",
        err.message,
      ),
    );
    ...
    logger.error("Get collectible by id error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to fetch collectible listing" });
  }
};
...
exports.updateCollectible = async (req, res) => {
  try {
    ...
    const listing = await Collectible.findById(req.params.id);
    if (!listing) {
      return res
        .status(404)
        .json({ success: false, message: "Collectible listing not found" });
    ...
    Object.assign(listing, { ...req.body, category: "Collectibles" });
    await listing.save();

    const updated = await Collectible.findById(listing._id).populate(
      "seller",
      "name profileImage",
    );
    ...
    try {
      await Promise.all([
        ListingCache.cacheListing("collectibles", listingObj),
        ListingCache.invalidateListCaches("collectibles"),
      ]);
    } catch (cacheErr) {
      logger.error(
        "[Cache] Collectibles update cache sync error:",
        cacheErr.message,
      );
    }
    ...
    logger.error("Update collectible error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to update collectible listing" });
  }
};

exports.deleteCollectible = async (req, res) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
      return res
        .status(400)
    ...
    try {
      await Promise.all([
        ListingCache.invalidateListCaches("collectibles"),
        ListingCache.invalidateListingCache("collectibles", req.params.id),
      ]);
    } catch (cacheErr) {
      logger.error(
        "[Cache] Collectibles delete cache sync error:",
        cacheErr.message,
      );
    }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Collectible listing deleted" });
  } catch (error) {
    logger.error("Delete collectible error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to delete collectible listing" });
  }
};
...
exports.getMyCollectibles = async (req, res) => {
  try {
    ...
    logger.error("Get my collectibles error:", error);
    return res.status(500).json({
      success: false,
      message: "Failed to fetch your collectible listings",
    });
  }
};

exports.getSavedCollectibles = async (req, res) => {
  try {
    const { page, limit } = parsePagination(req.query, { limit: 20 });

    const savedKey = `saved:collectibles:${req.user._id}:p${page}:l${limit}`;
    try {
      ...
      logger.warn("Saved collectibles cache read failed", {
    ...
    try {
      ...
      logger.warn("Saved collectibles cache write failed", {
        error: cacheErr.message,
      });
    }
    ...
    logger.error("Get saved collectibles error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to fetch saved collectibles" });
  }
};
...
exports.uploadImages = async (req, res) => {
  try {
    ...
    try {
      ...
      logger.error("[Cache] Error caching uploaded collectibles images:", cacheErr.message);
    }

    return res.status(200).json({ success: true, imageUrls });
  } catch (error) {
    logger.error("Upload collectibles images error:", error);
    return res
      .status(500)
...
exports.toggleSave = async (req, res) => {
  try {
    ...
    const listing = await Collectible.findById(req.params.id).select('savedBy').lean();
    if (!listing) {
      return res
        .status(404)
        .json({ success: false, message: "Collectible listing not found" });
    ...
    if (alreadySaved) {
      await Collectible.updateOne({ _id: req.params.id }, { $pull: { savedBy: userId } });
    } else {
      await Collectible.updateOne({ _id: req.params.id }, { $addToSet: { savedBy: userId } });
    }

    try {
      const pattern = `saved:collectibles:${userId}:*`;
      const keys = await redis.keys(pattern);
      if (keys.length) await redis.del(keys);
    } catch (cacheErr) {
      logger.warn("Toggle save collectibles cache clear failed", {
        error: cacheErr.message,
      });
    }
    ...
    logger.error("Toggle save collectible error:", error);
    return res
      .status(500)
