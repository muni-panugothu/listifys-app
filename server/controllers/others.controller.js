...
exports.createOther = async (req, res) => {
  try {
    ...
    const listing = await Other.create(payload);
    const populated = await Other.findById(listing._id).populate(
      "seller",
      "name profileImage",
    );
    ...
    logger.error("Create other listing error:", error);
    return res
      .status(500)
...
exports.getAllOthers = async (req, res) => {
  try {
    ...
    const cached = await ListingCache.getCachedListingList("others", queryKey);
    if (cached) {
    ...
    if (search && !(lat && lng)) {
      ...
      if (esResult) {
        ...
        Promise.all([
          ListingCache.cacheListingList('others', queryKey, esResult.docs, esResult.pagination),
          ListingCache.prefetchCategoryListings('others', esResult.docs),
          ListingCache.cacheSearchResults('others', search, esResult.docs, esResult.pagination),
    ...
    if (safePage > 1) {
      listings = await Other.find(filter, LIST_PROJECTION)
        .sort(sortOption)
        .skip(skip)
        .limit(safeLimit + 1)
      ...
      const [items, total] = await Promise.all([
        Other.find(filter, LIST_PROJECTION)
          .sort(sortOption)
          .limit(safeLimit)
          .populate("seller", "name profileImage")
          .lean(),
        Other.countDocuments(filter),
      ]);
      listings = items;
      pagination = {
        total,
    ...
    Promise.all([
      ListingCache.cacheListingList("others", queryKey, listings, pagination),
      ListingCache.prefetchCategoryListings("others", listings),
      search
        ? ListingCache.cacheSearchResults("others", search, listings, pagination)
        : null,
    ]).catch((err) =>
      logger.error(
        "[Cache] Background others cache write error:",
        err.message,
      ),
    );
    ...
    logger.error("Get others error:", error);
    return res
      .status(500)
...
exports.getOtherById = async (req, res) => {
  try {
    ...
    if (isObjectId) {
      const cached = await ListingCache.getCachedListing("others", param);
      if (cached) {
        viewCounter.recordView("others", param);
    ...
    const listing = isObjectId
      ? await Other.findById(param).populate(
          "seller",
          "name profileImage",
        )
      : await Other.findOne({ slug: param, status: "active" }).populate(
          "seller",
          "name profileImage",
        );
    ...
    viewCounter.recordView("others", listingId);
    ...
    ListingCache.cacheListing("others", listingObj).catch((err) =>
      logger.error(
        "[Cache] Background others detail cache error:",
        err.message,
      ),
    );
    ...
    logger.error("Get other by id error:", error);
    return res
      .status(500)
...
exports.updateOther = async (req, res) => {
  try {
    ...
    const listing = await Other.findById(req.params.id);
    if (!listing) {
      return res
        .status(404)
        .json({ success: false, message: "Listing not found" });
    ...
    Object.assign(listing, { ...req.body, category: "Others" });
    await listing.save();

    const updated = await Other.findById(listing._id).populate(
      "seller",
      "name profileImage",
    );
    ...
    try {
      await Promise.all([
        ListingCache.cacheListing("others", listingObj),
        ListingCache.invalidateListCaches("others"),
      ]);
    } catch (cacheErr) {
      logger.error(
        "[Cache] Others update cache sync error:",
        cacheErr.message,
      );
    }
    ...
    logger.error("Update other error:", error);
    return res
      .status(500)
...
exports.deleteOther = async (req, res) => {
  try {
    ...
    try {
      await Promise.all([
        ListingCache.invalidateListCaches("others"),
        ListingCache.invalidateListingCache("others", req.params.id),
      ]);
    } catch (cacheErr) {
      logger.error(
        "[Cache] Others delete cache sync error:",
        cacheErr.message,
      );
    }
    ...
    logger.error("Delete other error:", error);
    return res
      .status(500)
...
exports.getMyOthers = async (req, res) => {
  try {
    ...
    logger.error("Get my others error:", error);
...
exports.getSavedOthers = async (req, res) => {
  try {
    const { page, limit } = parsePagination(req.query, { limit: 20 });

    const savedKey = `saved:others:${req.user._id}:p${page}:l${limit}`;
    try {
      ...
      logger.warn("Saved others cache read failed", {
    ...
    try {
      ...
      logger.warn("Saved others cache write failed", {
        error: cacheErr.message,
      });
    }
    ...
    logger.error("Get saved others error:", error);
    return res
      .status(500)
      .json({ success: false, message: "Failed to fetch saved listings" });
  }
};
...
exports.uploadImages = async (req, res) => {
  try {
    ...
    logger.error("Upload other images error:", error);
    return res
      .status(500)
...
exports.toggleSave = async (req, res) => {
  try {
    ...
    // Check current state with lean query (no full document hydration)
    const listing = await Other.findById(listingId).select('savedBy').lean();
    if (!listing) {
      return res
        .status(404)
        .json({ success: false, message: "Listing not found" });
    ...
    if (alreadySaved) {
      await Other.updateOne({ _id: listingId }, { $pull: { savedBy: userId } });
    } else {
      await Other.updateOne({ _id: listingId }, { $addToSet: { savedBy: userId } });
    }

    return res.status(200).json({
      success: true,
      saved: !alreadySaved,
      message: !alreadySaved ? "Listing saved" : "Listing unsaved",
    });
  } catch (error) {
    logger.error("Toggle save other error:", error);
    return res
      .status(500)
