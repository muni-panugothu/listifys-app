...
const OTHER_SUBCATEGORIES = ["Other Items"];

const createOtherSchema = Joi.object({
  title: Joi.string().trim().max(200).required(),
  description: Joi.string().trim().min(20).required(),
  price: Joi.number().min(0).required(),
  category: Joi.string().valid("Others").default("Others"),
  subcategory: Joi.string()
    .valid(...OTHER_SUBCATEGORIES)
    .required()
    .messages({
      "any.only": `Subcategory must be one of: ${OTHER_SUBCATEGORIES.join(", ")}`,
    }),
...
const queryOtherSchema = Joi.object({
...
module.exports = {
  validateCreateOther: validate(createOtherSchema),
  validateUpdateOther: validate(updateOtherSchema),
  validateQueryOther: validate(queryOtherSchema, "query"),
