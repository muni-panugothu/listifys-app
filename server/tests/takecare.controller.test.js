...
jest.mock('../models/takecare.model', () => ({
...
const controller = require('../controllers/takecare.controller');

describe('🏥 TAKECARE CONTROLLER TESTS', () => {
  let req, res;
  beforeEach(() => { jest.clearAllMocks(); req = createMockReq(); res = createMockRes(); });

  describe('1. getAllTakeCare (Public)', () => {
    test('Should fetch paginated listings', async () => {
      req.query = { page: 1, limit: 10 };
      mockFind.mockResolvedValue([{ _id: '1', title: 'Pet Sitter', images: ['i.jpg'] }, { _id: '2', title: 'Babysitter', images: ['i2.jpg'] }]);
      mockCountDocuments.mockResolvedValue(2);
      await controller.getAllTakeCare(req, res);
  ...
  describe('2. getTakeCareById (Public)', () => {
    ...
    test('Should return listing if valid ID', async () => {
      ...
      mockFindByIdResult.mockReturnValue({ _id: id, title: 'Pet Sitter', images: ['i.jpg'], status: 'active', toObject() { return { ...this }; } });
      await controller.getTakeCareById(req, res);
      expect(res.statusCode).toBe(200); expect(res._json.listing.title).toBe('Pet Sitter');
    });
  });

  describe('3. getMyTakeCare (Protected)', () => {
  ...
  describe('4. deleteTakeCare (Protected)', () => {
    ...
    test('Should delete if owner', async () => {
      ...
      require('../models/takecare.model').findById.mockResolvedValueOnce(listing);
      await controller.deleteTakeCare(req, res);
      expect(res.statusCode).toBe(200); expect(require('../models/takecare.model').findByIdAndDelete).toHaveBeenCalled();
    });
    test('Should forbid if not owner', async () => {
      const id = new mongoose.Types.ObjectId().toString(); req.params = { id }; req.user = { _id: 'attacker' };
      require('../models/takecare.model').findById.mockResolvedValueOnce({ _id: id, seller: { toString: () => 'user-123' }, images: [], toObject() { return { ...this }; }, deleteOne: jest.fn() });
      await controller.deleteTakeCare(req, res);
