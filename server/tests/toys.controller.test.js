...
describe('🧸 TOYS CONTROLLER TESTS', () => {
  let req, res;
  beforeEach(() => { jest.clearAllMocks(); req = createMockReq(); res = createMockRes(); });

  describe('1. getAllToys (Public)', () => {
    test('Should fetch paginated listings', async () => {
      req.query = { page: 1, limit: 10 };
      mockFind.mockResolvedValue([{ _id: '1', title: 'LEGO Set', images: ['i.jpg'] }, { _id: '2', title: 'Board Game', images: ['i2.jpg'] }]);
      mockCountDocuments.mockResolvedValue(2);
      await controller.getAllToys(req, res);
  ...
  describe('2. getToyById (Public)', () => {
    ...
    test('Should return listing if valid ID', async () => {
      ...
      mockFindByIdResult.mockReturnValue({ _id: id, title: 'LEGO Set', images: ['i.jpg'], status: 'active', toObject() { return { ...this }; } });
      await controller.getToyById(req, res);
      expect(res.statusCode).toBe(200); expect(res._json.listing.title).toBe('LEGO Set');
    });
  });

  describe('3. getMyToys (Protected)', () => {
  ...
  describe('4. deleteToy (Protected)', () => {
    ...
    test('Should delete if owner', async () => {
      ...
      require('../models/toy.model').findById.mockResolvedValueOnce(listing);
      await controller.deleteToy(req, res);
    ...
    test('Should forbid if not owner', async () => {
      ...
      require('../models/toy.model').findById.mockResolvedValueOnce({ _id: id, seller: { toString: () => 'user-123' }, images: [], toObject() { return { ...this }; }, deleteOne: jest.fn() });
      await controller.deleteToy(req, res);
