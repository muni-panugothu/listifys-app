...
describe('💼 JOBS CONTROLLER TESTS', () => {
  ...
  describe('1. getAllJobs (Public)', () => {
    test('Should fetch paginated listings', async () => {
      req.query = { page: 1, limit: 10 };
      mockFind.mockResolvedValue([{ _id: '1', title: 'Full Stack Dev', images: ['i.jpg'] }, { _id: '2', title: 'UI Designer', images: ['i2.jpg'] }]);
  ...
  describe('2. getJobById (Public)', () => {
    ...
    test('Should return listing if valid ID', async () => {
      ...
      mockFindByIdResult.mockReturnValue({ _id: id, title: 'Full Stack Dev', images: ['i.jpg'], status: 'active', toObject() { return { ...this }; } });
      await controller.getJobById(req, res);
      expect(res.statusCode).toBe(200); expect(res._json.listing.title).toBe('Full Stack Dev');
    });
  });

  describe('3. getMyJobs (Protected)', () => {
  ...
  describe('4. deleteJob (Protected)', () => {
    ...
    test('Should return 404 if not found', async () => {
      ...
      require('../models/job.model').findById.mockResolvedValueOnce(null);
      await controller.deleteJob(req, res);
      expect(res.statusCode).toBe(404);
    });
    test('Should delete if owner', async () => {
      ...
      require('../models/job.model').findById.mockResolvedValueOnce(listing);
      await controller.deleteJob(req, res);
      expect(res.statusCode).toBe(200); expect(require('../models/job.model').findByIdAndDelete).toHaveBeenCalled();
    });
    test('Should forbid if not owner', async () => {
      const id = new mongoose.Types.ObjectId().toString(); req.params = { id }; req.user = { _id: 'attacker' };
      require('../models/job.model').findById.mockResolvedValueOnce({ _id: id, seller: { toString: () => 'user-123' }, images: [], toObject() { return { ...this }; }, deleteOne: jest.fn() });
      await controller.deleteJob(req, res);
