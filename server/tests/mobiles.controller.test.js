...
const controller = require('../controllers/mobiles.controller');

describe('📱 MOBILES CONTROLLER TESTS', () => {
  let req, res;
  beforeEach(() => { jest.clearAllMocks(); req = createMockReq(); res = createMockRes(); });

  describe('1. getAllMobiles (Public)', () => {
    test('Should fetch paginated listings', async () => {
      req.query = { page: 1, limit: 10 };
      mockFind.mockResolvedValue([{ _id: '1', title: 'iPhone 15', images: ['i.jpg'] }, { _id: '2', title: 'Samsung S24', images: ['i2.jpg'] }]);
      mockCountDocuments.mockResolvedValue(2);
      await controller.getAllMobiles(req, res);
  ...
  describe('2. getMobileById (Public)', () => {
    ...
    test('Should return listing if valid ID', async () => {
      ...
      mockFindByIdResult.mockReturnValue({ _id: id, title: 'iPhone 15', images: ['i.jpg'], status: 'active', toObject() { return { ...this }; } });
      await controller.getMobileById(req, res);
      expect(res.statusCode).toBe(200); expect(res._json.listing.title).toBe('iPhone 15');
    });
  });

  describe('3. getMyMobiles (Protected)', () => {
  ...
  describe('4. deleteMobile (Protected)', () => {
    ...
    test('Should delete if owner', async () => {
      ...
      require('../models/mobile.model').findById.mockResolvedValueOnce(listing);
      await controller.deleteMobile(req, res);
    ...
    test('Should forbid if not owner', async () => {
      ...
      require('../models/mobile.model').findById.mockResolvedValueOnce({ _id: id, seller: { toString: () => 'user-123' }, images: [], toObject() { return { ...this }; }, deleteOne: jest.fn() });
      await controller.deleteMobile(req, res);
