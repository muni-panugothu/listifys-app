...
describe('🎉 EVENTS CONTROLLER TESTS', () => {
  ...
  describe('1. getAllEvents (Public)', () => {
    test('Should fetch paginated listings', async () => {
      req.query = { page: 1, limit: 10 };
      mockFind.mockResolvedValue([{ _id: '1', title: 'Tech Meetup', images: ['i.jpg'] }, { _id: '2', title: 'Music Fest', images: ['i2.jpg'] }]);
  ...
  describe('2. getEventById (Public)', () => {
    ...
    test('Should return listing if valid ID', async () => {
      ...
      mockFindByIdResult.mockReturnValue({ _id: id, title: 'Tech Meetup', images: ['i.jpg'], status: 'active', toObject() { return { ...this }; } });
      await controller.getEventById(req, res);
      expect(res.statusCode).toBe(200); expect(res._json.listing.title).toBe('Tech Meetup');
    });
  });

  describe('3. getMyEvents (Protected)', () => {
    test('Should return user listings', async () => {
      ...
      await controller.getMyEvents(req, res);
      expect(res.statusCode).toBe(200); expect(res._json.success).toBe(true);
    });
  });

  describe('4. deleteEvent (Protected)', () => {
    ...
    test('Should return 404 if not found', async () => {
      ...
      require('../models/event.model').findById.mockResolvedValueOnce(null);
      await controller.deleteEvent(req, res);
      expect(res.statusCode).toBe(404);
    });
    test('Should delete if owner', async () => {
      ...
      require('../models/event.model').findById.mockResolvedValueOnce(listing);
      await controller.deleteEvent(req, res);
      expect(res.statusCode).toBe(200); expect(require('../models/event.model').findByIdAndDelete).toHaveBeenCalled();
    });
    test('Should forbid if not owner', async () => {
      const id = new mongoose.Types.ObjectId().toString(); req.params = { id }; req.user = { _id: 'attacker' };
      require('../models/event.model').findById.mockResolvedValueOnce({ _id: id, seller: { toString: () => 'user-123' }, images: [], toObject() { return { ...this }; }, deleteOne: jest.fn() });
      await controller.deleteEvent(req, res);
